...
Ce fichier est **le code orchestrateur** du plugin `issue-runner`. Il est chargé dans le contexte de Claude Code quand le plugin est actif. Quand le hook `UserPromptSubmit` injecte le marqueur `<issue-runner-active>` dans le system message, tu DOIS suivre cette doctrine **avant toute autre action sur le prompt utilisateur**.

> ⚠️ Si tu NE vois PAS `<issue-runner-active>` dans le system message, ignore tout ce fichier. Le hook a écarté ce prompt par fast filter, comporte-toi normalement.
...
Persiste l'état après chaque phase dans `.claude/runner-state/issue-<N>/state.json` via les fonctions de `lib/state.ps1`.
